...
const { streamResponse, smartRoute, calculateCost, hasApiKey } = require('../llm/router');
...
router.post('/', async (req, res) => {
  ...
  const promises = available.map(async (modelId) => {
    try {
      ...
      trackUsage(modelId, usage.input, usage.output, cost, 'battle');
